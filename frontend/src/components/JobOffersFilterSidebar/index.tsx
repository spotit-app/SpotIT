...
import { ReadWorkExperience, ReadWorkMode } from 'types/company';
import { useForeignLanguages, useTechSkills, useWorkModes, useWorkExperiences } from 'hooks';
import { FilterButton } from '../FilterButton';
import { Loading } from '../Loading';
import { Button } from '../Button';
import icons from 'assets/icons';

function JobOffersFilterSidebar() {
  ...
  useEffect(() => {
    ...
    setSelectedWorkExperiences(params.getAll('experienceNameIds'));
    setSelectedWorkModes(params.getAll('workModeIds'));
  }, [params]);
  ...
  const onFilterClick = () => {
    setParams({
      ...
      experienceNameIds: selectedWorkExperiences,
      workModeIds: selectedWorkModes
    });
  };
  ...
  const workModesToChooseFrom = workModes?.map((workMode: ReadWorkMode) => (
    <FilterButton
      key={workMode.id}
      entity={workMode}
      onChange={() => setSelectedWorkModes((arr) => toggleButton(arr, workMode.id.toString()))}
      isChecked={selectedWorkModes.includes(workMode.id.toString())}
    />
  ));

  const workExperienceToChooseFrom = workExperiences?.map((experience: ReadWorkExperience) => (
    <FilterButton
      key={experience.id}
      entity={experience}
      onChange={() =>
        setSelectedWorkExperiences((arr) => toggleButton(arr, experience.id.toString()))
      }
      isChecked={selectedWorkExperiences.includes(experience.id.toString())}
    />
  ));
        ...
        <div className="flex flex-col items-end">
          ...
          <div className="w-80 sm:w-96  flex flex-col items-center gap-x-4">
            <div role="alert" className="text-center mb-4 alert border-2 border-primary rounded-lg">
              ...
              <h1 className="italic text-lg">Jakiej pracy szukasz?</h1>
            </div>
            ...
            workModesIsPending ? (
              ...
              <>
                ...
                <h3 className="mt-5 mb-3 text-lg self-start font-medium">Tryb pracy:</h3>
                <ul className="flex flex-row justify-center flex-wrap">{workModesToChooseFrom}</ul>
                <h3 className="mt-5 mb-3 text-lg self-start font-medium">Doświadczenie:</h3>
                <ul className="flex flex-row justify-center flex-wrap">
                  {workExperienceToChooseFrom}
                </ul>
