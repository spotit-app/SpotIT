...
class ForeignLanguageNameIntegrationTest {
    ...
    void shouldDeleteForeignLanguageName() throws Exception {
        ...
        // then
        result.andExpect(status().isOk())
                .andExpect(content().string(getDeletedEntityMessage(foreignLanguageNameToDelete.getId())));
    ...
    void shouldReturnErrorWhenDeletedForeignLanguageNameNotFound() throws Exception {
        ...
        // when
        var result = mockMvc.perform(delete(FOREIGN_LANGUAGE_NAME_API + "/" + foreignLanguageNameId).with(jwt()));
