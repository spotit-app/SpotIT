...
import static com.spotit.backend.employee.referenceData.techSkillName.TechSkillNameUtils.generateTechSkillNameList;
...
class TechSkillNameServiceTest {
    ...
    void shouldReturnTechSkillNameById() {
        ...
        when(techSkillNameRepository.findById(1))
                .thenReturn(Optional.of(foundTechSkillName));
    ...
    void shouldReturnTechSkillNameByName() {
        ...
        when(techSkillNameRepository.findByName(foundTechSkillNameName))
                .thenReturn(Optional.of(foundTechSkillName));
    ...
    void shouldThrowExceptionWhenTechSkillNameNotFound() {
        ...
        var exception = assertThrows(
                ...
                () -> techSkillNameServiceImpl.getById(1));
    ...
    @Test
    void shouldReturnTechSkillNameWhenCreatedSuccesfully() {
        // given
        ...
        when(storageService.uploadFile(
                techSkillNameLogo,
                "logos",
                techSkillNameName + "-logo"))
                .thenReturn("logoUrl");
        when(techSkillNameRepository.save(any()))
                .thenAnswer(i -> i.getArguments()[0]);

        // when
        var result = techSkillNameServiceImpl.create(
                techSkillNameName,
                techSkillNameLogo);
    ...
    void shouldThrowExceptionWhenErrorCreatingTechSkillName() {
        ...
        var exception = assertThrows(
                ...
                () -> techSkillNameServiceImpl.create(
                        techSkillNameName,
                        techSkillNameLogo));
    ...
    void shouldDeleteTechSkillNameById() {
        ...
        when(techSkillNameRepository.findById(1))
                .thenReturn(Optional.of(techSkillNameToDelete));

        // when
        techSkillNameServiceImpl.delete(1);

        // then
        verify(techSkillNameRepository, times(1))
                .findById(techSkillNametoDeleteId);
        verify(storageService, times(1))
                .deleteFile(any());
        verify(techSkillNameRepository, times(1))
                .delete(techSkillNameToDelete);
    }
    ...
    void shouldReturnModifiedTechSkillName() {
        ...
        when(techSkillNameRepository.findById(techSkillNameToModifyId))
                .thenReturn(Optional.of(currentTechSkillName));
        when(techSkillNameRepository.save(any()))
                .thenAnswer(i -> i.getArguments()[0]);

        // when
        var result = techSkillNameServiceImpl.update(
                techSkillNameToModifyId,
                modifiedTechSkillNameName,
                modifiedTechSkillNameLogo);

        // then
        verify(techSkillNameRepository, times(1))
                .findById(techSkillNameToModifyId);
        verify(storageService, times(1)).uploadFile(
                any(), any(), any());
        verify(techSkillNameRepository, times(1))
                .save(any());
    ...
    void shouldReturnPartiallyModifiedTechSkillName() {
        ...
        var modifiedTechSkillName = TechSkillName.builder()
                .id(1)
                .name("modifiedName")
                .build();
